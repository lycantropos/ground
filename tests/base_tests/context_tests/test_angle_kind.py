...
def test_same_endpoints(
    ...
    assert context.angle_kind(end, start, start) is (
        Kind.RIGHT if start == end else Kind.ACUTE
    )
...
def test_endpoints_permutations(
    ...
    assert result is context.angle_kind(
        vertex, second_ray_point, first_ray_point
    )
