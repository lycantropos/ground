...
def test_basic(
    context_with_point_and_steps: tuple[
        Context[ScalarT], Point[ScalarT], ScalarT, ScalarT
    ],
) -> None:
...
def test_round_trip(
    ...
    assert context.translate_point(
        result, -step_x, -step_y
    ) == context.translate_point(point, context.zero, context.zero)
