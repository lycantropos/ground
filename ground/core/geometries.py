...
class Multipoint(Generic[hints.ScalarT]):
    ...
    def __eq__(self, other: Any) -> Any:
        return (
            are_sequences_equivalent(self.points, other.points)
...
class Segment(Generic[hints.ScalarT]):
    ...
    def __new__(
        cls, start: hints.Point[hints.ScalarT], end: hints.Point[hints.ScalarT]
    ) -> Self:
        self = super().__new__(cls)
...
class Multisegment(Generic[hints.ScalarT]):
    ...
    def __new__(cls, segments: Sequence[hints.Segment[hints.ScalarT]]) -> Self:
        self = super().__new__(cls)
...
class Contour(Generic[hints.ScalarT]):
    ...
    def __new__(cls, vertices: Sequence[hints.Point[hints.ScalarT]]) -> Self:
        self = super().__new__(cls)
