...
def point_squared_distance(start: Point,
    ...
    return ((square(segment_squared_norm * (Expansion(start.x)
                                            - Expansion(point.x))
                    + end_factor * (Expansion(end.x) - Expansion(start.x)))
             + square(segment_squared_norm * (Expansion(start.y)
                                              - Expansion(point.y))
                      + end_factor * (Expansion(end.y) - Expansion(start.y)))
             if (abs(segment_squared_norm - end_factor)
                 < abs(segment_squared_norm - start_factor))
             else (square(segment_squared_norm * (Expansion(end.x)
                                                  - Expansion(point.x))
                          + start_factor * (Expansion(start.x)
                                            - Expansion(end.x)))
                   + square(segment_squared_norm * (Expansion(end.y)
                                                    - Expansion(point.y))
                            + start_factor * (Expansion(start.y)
                                              - Expansion(end.y)))))
